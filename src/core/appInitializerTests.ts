...
if (import.meta.env.DEV) {
  ...
  setTimeout(() => {
    window.__APP_TESTS__ = {
      async runIntegrationTests(config = { useMockData: true, enableDebug: true }) {
        const tests = new AppInitializerTests(config)
        return await tests.runAllIntegrationTests()
      },

      async testProductRecipeIntegration() {
        const tests = new AppInitializerTests({})
        return await tests.testProductRecipeIntegration()
      },

      async testPerformance() {
        const tests = new AppInitializerTests({})
        return await tests.testLoadingPerformance()
      },

      generateReport(config = { useMockData: true, enableDebug: true }) {
        const tests = new AppInitializerTests(config)
        const report = tests.generateIntegrationReport()
        console.table(report.stores)
        console.log('Full report:', report)
        return report
      }
    }

    console.log('\n🧪 App Integration Tests loaded! Try:')
    console.log('  • window.__APP_TESTS__.runIntegrationTests()')
    console.log('  • window.__APP_TESTS__.testProductRecipeIntegration()')
    console.log('  • window.__APP_TESTS__.generateReport()')
  }, 3000)
}
