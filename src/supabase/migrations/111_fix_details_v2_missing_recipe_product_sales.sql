-- Migration: 111_fix_details_v2_missing_recipe_product_sales
-- Description: Add missing recipe_product_sales path to variance details function
-- Date: 2026-02-01
-- Author: Claude
--
-- PROBLEM:
-- get_product_variance_details_v2 is missing the path:
-- Menu → Recipe → Product (product directly in recipe, not via preparation)
--
-- This causes Sales discrepancy between report_v3 and details_v2:
-- - report_v3: 22,060 ml (includes all 3 paths)
-- - details_v2: 1,270 ml (missing recipe_product_sales path)
--
-- FIX:
-- Add recipe_product_sales CTE to capture products used directly in recipes
...
BEGIN
  ...
  -- NEW Path 2: Menu → Recipe → Product (product directly in recipe)
  ...
  -- UPDATED: Include all 4 sales paths
  all_menu_item_sales AS (
    ...
    UNION ALL
    SELECT menu_item_id, menu_item_name, variant_name, quantity_sold, product_used, sales_type,
           via_recipe as preparation_name  -- using recipe name as reference
  ...
  -- NEW: Calculate direct sales from recipes (not via preparations)
...
'Product Variance Detail V2.5 - includes missing recipe_product_sales path for consistent Sales calculation';
