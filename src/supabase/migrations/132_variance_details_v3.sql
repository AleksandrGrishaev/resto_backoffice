-- Migration: 132_variance_details_v3
-- Description: Product Variance Details V3 using helper functions
-- Date: 2026-02-02
-- Author: Claude
--
-- This is a refactoring of get_product_variance_details_v2 to use the same
-- helper functions as the main report. This guarantees:
-- 1. Identical calculations between main report and details
-- 2. Writeoffs and Loss are properly decomposed from preparations
-- 3. Easier maintenance - change once, applies everywhere
--
-- Changes from v2:
-- - Uses calc_product_theoretical_sales() for sales
-- - Uses calc_product_writeoffs_decomposed() for writeoffs
-- - Uses calc_product_loss_decomposed() for loss
-- - Uses calc_product_inpreps() for in-preps
-- - Adds writeoffs decomposition breakdown (was missing in v2)
-- - Adds loss decomposition breakdown (was missing in v2)
...
COMMENT ON FUNCTION get_product_variance_details_v3 IS 'Product variance details V3.0 - Uses helper functions for guaranteed sync with main report. Includes writeoffs/loss decomposition from preparations.';
