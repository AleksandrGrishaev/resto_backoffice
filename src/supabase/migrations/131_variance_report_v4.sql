-- Migration: 131_variance_report_v4
-- Description: Product Variance Report V4 - Clean refactoring of v3
-- Date: 2026-02-02
-- Author: Claude
--
-- This is a clean refactoring of get_product_variance_report_v3.
-- Uses the same CTE structure but organized more clearly.
-- The helper functions are available for single-product queries (details_v3).
--
-- Key points:
-- - Same calculations as v3 (verified matching results)
-- - Cleaner CTE organization
...
BEGIN
  ...
  -- Recursive prep decomposition for sales
  ...
  prep_writeoffs_decomposed AS (
    WITH RECURSIVE wo_prep_tree AS (
      ...
      SELECT wpt.root_prep_id, pi.ingredient_id as current_prep_id,
        (wpt.consumed_qty * pi.quantity / NULLIF(p.output_quantity * COALESCE(p.portion_size, 1), 0))::NUMERIC as consumed_qty, wpt.depth + 1
  ...
  prep_loss_decomposed AS (
    WITH RECURSIVE loss_prep_tree AS (
      ...
      SELECT lpt.root_prep_id, pi.ingredient_id as current_prep_id,
        (lpt.consumed_qty * pi.quantity / NULLIF(p.output_quantity * COALESCE(p.portion_size, 1), 0))::NUMERIC as consumed_qty, lpt.depth + 1
...
'Product Variance Report V4.0 - Clean refactoring of v3 with identical calculations';
