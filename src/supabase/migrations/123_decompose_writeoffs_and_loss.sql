-- Migration 123: Decompose writeoffs and loss for preparations
-- Problem: Write-offs and Loss only show direct product operations, not decomposed prep operations
-- Solution: When sales_consumption/loss is recorded for a preparation, decompose it to products
-- Version: v4.8
--
-- EXAMPLE:
-- When sales_consumption writes off 1kg of dough (preparation):
--   → Show 500g flour, 480g water, 20g yeast in Write-offs
--
-- When dough expires (loss):
--   → Show 500g flour, 480g water, 20g yeast in Loss
  ...
  prep_writeoffs_decomposed AS (
    WITH RECURSIVE wo_prep_tree AS (
      ...
      SELECT wpt.root_prep_id,
             ...
             (wpt.consumed_qty * pi.quantity / NULLIF(p.output_quantity, 0))::NUMERIC as consumed_qty,
             wpt.depth + 1
  ...
  prep_loss_decomposed AS (
    WITH RECURSIVE loss_prep_tree AS (
      ...
      SELECT lpt.root_prep_id,
             pi.ingredient_id as current_prep_id,
             (lpt.consumed_qty * pi.quantity / NULLIF(p.output_quantity, 0))::NUMERIC as consumed_qty,
             lpt.depth + 1
...
'Product Variance Report V4.8 - Write-offs and Loss decomposed from preparations to products';
