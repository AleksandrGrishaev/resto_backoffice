-- Migration 117: Add sales breakdown (direct, viaRecipes, topMenuItems) to details_v2
-- Problem: UI shows "No sales in this period" because breakdown data was missing
-- Solution: Add direct, viaRecipes, viaPreparations breakdown + topMenuItems array
-- Version: v2.9
...
BEGIN
  ...
  all_menu_item_sales AS (
    ...
    SELECT * FROM recipe_product_sales UNION ALL
    SELECT * FROM prep_direct_menu_sales UNION ALL
    SELECT * FROM prep_recipe_menu_sales
  ),
  ...
  -- Top menu items for display (limit 10)
  top_menu_items AS (
