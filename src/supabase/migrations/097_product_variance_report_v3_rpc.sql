...
-- Description: Product Variance Report V3.1 with inventory_snapshots for Opening
--              and products_in_preparations tracking
-- Date: 2026-01-27
-- Updated: Added inventory_snapshots support and inPreps calculation
...
BEGIN
  ...
  -- Opening date is the day before period start
  ...
  -- 5. Join order items with compositions
  order_compositions AS (
    SELECT oid.menu_item_id, oid.variant_id, oid.order_qty,
           vc.composition_id, vc.composition_type, vc.composition_qty, vc.composition_unit
  ...
  -- 6. Direct products from compositions
  direct_products_from_orders AS (
    SELECT oc.composition_id::UUID as product_id, SUM(oc.order_qty * oc.composition_qty) as qty
    FROM order_compositions oc WHERE oc.composition_type = 'product' GROUP BY oc.composition_id
  ),

  -- 7. Recipe components
  recipe_comp AS (
    SELECT rc.recipe_id, rc.component_id, rc.component_type, rc.quantity as component_qty,
           rc.unit as component_unit, r.portion_size as recipe_portion_size
    FROM recipe_components rc JOIN recipes r ON r.id = rc.recipe_id WHERE r.is_active = true
  ),

  -- 8. Products from recipes
  products_from_recipes AS (
    ...
    WHERE oc.composition_type = 'recipe' AND rc.component_type = 'product'
  ...
  preps_from_recipes AS (
    ...
    FROM order_compositions oc
    JOIN recipe_comp rc ON rc.recipe_id = oc.composition_id::UUID
    WHERE oc.composition_type = 'recipe' AND rc.component_type = 'preparation'
    GROUP BY rc.component_id
  ),
  ...
  products_from_preparations AS (
    SELECT pi.ingredient_id as product_id,
           SUM(ap.qty * pi.quantity / NULLIF(p.output_quantity, 0)) as qty
    FROM all_preps ap
  ...
  -- 13. THEORETICAL SALES
  theoretical_sales AS (
    SELECT product_id, SUM(qty) as qty FROM (
      SELECT * FROM direct_products_from_orders UNION ALL
      SELECT * FROM products_from_recipes UNION ALL
      SELECT * FROM products_from_preparations
    ) combined GROUP BY product_id
  ),

  -- 14. Opening stock from snapshots (NEW: uses inventory_snapshots table)
  opening_stock AS (
    SELECT item_id, quantity as qty, total_cost as amount
    FROM inventory_snapshots
    WHERE snapshot_date = v_opening_date AND item_type = 'product'
  ),

  -- 15. Products in active preparation batches (NEW: frozen in preps)
  products_in_active_preps AS (
    SELECT
      pi.ingredient_id::TEXT as item_id,
      SUM(pb.current_quantity * pi.quantity / NULLIF(p.output_quantity, 0)) as qty,
      SUM(pb.current_quantity * pi.quantity / NULLIF(p.output_quantity, 0) *
    ...
    JOIN preparation_ingredients pi ON pi.preparation_id = p.id
    WHERE pb.status = 'active' AND pb.current_quantity > 0 AND pi.type = 'product'
    GROUP BY pi.ingredient_id
  ),
  ...
  direct_loss AS (
    ...
    FROM storage_operations so, LATERAL jsonb_array_elements(so.items) AS item
    WHERE so.operation_type = 'write_off' AND so.operation_date >= p_start_date AND so.operation_date < p_end_date
      AND so.status = 'confirmed' AND (so.write_off_details->>'reason') IN ('expired', 'spoiled', 'other')
      AND (item->>'itemType') = 'product'
    GROUP BY (item->>'itemId')
  ),
  ...
  prep_recipes AS (
    SELECT p.id as preparation_id, p.output_quantity as prep_output_qty,
  ...
  traced_sales_writeoffs AS (
    SELECT pr.product_id::TEXT as item_id,
           SUM(CASE WHEN pr.prep_output_qty > 0 THEN (ps.qty / pr.prep_output_qty) * pr.recipe_product_qty ELSE 0 END) as qty,
           SUM(CASE WHEN ps.qty > 0 THEN (ps.cost / ps.qty) *
               CASE WHEN pr.prep_output_qty > 0 THEN (ps.qty / pr.prep_output_qty) * pr.recipe_product_qty ELSE 0 END
  ...
  traced_losses AS (
    SELECT pr.product_id::TEXT as item_id,
           SUM(CASE WHEN pr.prep_output_qty > 0 THEN (pl.qty / pr.prep_output_qty) * pr.recipe_product_qty ELSE 0 END) as qty,
           SUM(CASE WHEN pl.qty > 0 THEN (pl.cost / pl.qty) *
               CASE WHEN pr.prep_output_qty > 0 THEN (pl.qty / pr.prep_output_qty) * pr.recipe_product_qty ELSE 0 END
               ELSE 0 END) as amount
  ...
  -- 26. Products with preps flag
  products_with_preps AS (
    SELECT DISTINCT pr.product_id::TEXT as item_id FROM prep_recipes pr
  ),
  ...
  combined AS (
    SELECT
      ...
      COALESCE(pc.qty, 0) as production_qty, COALESCE(pc.amount, 0) as production_amount,
      ...
      (COALESCE(os.qty, 0) + COALESCE(r.qty, 0) - COALESCE(ts.qty, 0)
        - COALESCE(dl.qty, 0) - COALESCE(tl.qty, 0) - COALESCE(cs.qty, 0) - COALESCE(pip.qty, 0)) as variance_qty,
      (COALESCE(os.amount, 0) + COALESCE(r.amount, 0) - COALESCE(ts.qty, 0) * pi.avg_cost
        - COALESCE(dl.amount, 0) - COALESCE(tl.amount, 0) - COALESCE(cs.amount, 0) - COALESCE(pip.amount, 0)) as variance_amount
    ...
    WHERE (COALESCE(os.qty, 0) > 0 OR COALESCE(r.qty, 0) > 0 OR COALESCE(ts.qty, 0) > 0 OR
           COALESCE(dsw.qty, 0) > 0 OR COALESCE(tsw.qty, 0) > 0 OR COALESCE(dl.qty, 0) > 0 OR
           COALESCE(tl.qty, 0) > 0 OR COALESCE(cs.qty, 0) > 0 OR COALESCE(pip.qty, 0) > 0)
  ...
  SELECT jsonb_build_object(
    ...
    'byDepartment', jsonb_build_object(
      'kitchen', (SELECT jsonb_build_object(
        'count', COUNT(*), 'theoreticalSalesAmount', ROUND(COALESCE(SUM(theoretical_sales_amount), 0), 2),
    ...
    'items', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'unit', unit, 'department', department, 'hasPreparations', has_preparations,
      ...
      'stockTotal', jsonb_build_object('quantity', ROUND(closing_qty + in_preps_qty, 3), 'amount', ROUND(closing_amount + in_preps_amount, 2)),
      ...
      'salesWriteoffDiff', jsonb_build_object('quantity', ROUND(theoretical_sales_qty - writeoff_sales_qty, 3),
...
COMMENT ON FUNCTION get_product_variance_report_v3 IS 'Product Variance Report V3.1: Uses inventory_snapshots for Opening, includes products in preparations';
