-- Migration: 106_add_sales_breakdown_to_variance_details_v2
-- Description: Add detailed menu items breakdown for sales via preparations
-- Date: 2026-01-28
-- Author: Claude
--
-- PROBLEM:
-- For products like Tuna Lion that are only sold via preparations (not directly),
-- the "Sales (Theoretical)" section in the dialog shows "No sales in this period"
-- even though there ARE sales via preps (5,400g).
--
-- ROOT CAUSE:
-- topMenuItems only includes direct product sales and recipe sales,
-- but NOT menu items that use preparations containing this product.
--
-- FIX:
-- Add new CTE `menu_items_via_preps` to track which menu items consume
-- preparations that contain this product, and include them in topMenuItems.
...
BEGIN
  ...
  -- Direct product sales (product directly in menu item composition)
  ...
  -- Recipe product sales (product in recipe that menu item uses)
  ...
  -- Preparations that contain this product
  prep_recipes AS (
  ...
  -- NEW: Menu items that use preparations directly
  ...
  -- NEW: Menu items that use recipes which contain preparations
  ...
  -- UPDATED: Combine all menu item sales (direct, via recipe, via prep, via recipe->prep)
  ...
  menu_items_aggregated AS (
    SELECT
      ...
      -- Show prep name if all sales are via same prep, otherwise NULL
      CASE
  ...
  SELECT jsonb_build_object(
    ...
    'sales', jsonb_build_object(
      ...
      -- UPDATED: Include via_preparation field in topMenuItems
...
-- Add comment
COMMENT ON FUNCTION get_product_variance_details_v2 IS 'Enhanced product variance details with full breakdown and drill-down capabilities (v2.2 - added menu items breakdown for sales via preparations)';
