-- Migration 120: Add writeoffs (production_consumption) to report_v3
-- Shows actual write-offs from storage_operations for comparison with theoretical sales
-- Version: v4.6
...
BEGIN
  ...
  -- Shows actual write-offs for comparison with theoretical sales
  -- =============================================
  ...
  -- Product report with sales breakdown and writeoffs
  product_report AS (
    SELECT
      ...
      -- Actual write-offs (production_consumption)
      COALESCE(pw.qty, 0) as writeoffs_qty, COALESCE(pw.amount, 0) as writeoffs_amount,
      -- Loss and gain
  ...
  SELECT jsonb_build_object(
    ...
    'products', (SELECT jsonb_agg(jsonb_build_object(
      ...
      -- Sales-Writeoff difference (positive = under-written-off, negative = over-written-off)
