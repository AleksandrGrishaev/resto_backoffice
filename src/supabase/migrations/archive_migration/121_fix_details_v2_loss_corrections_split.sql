-- Migration 121: Fix details_v2 loss calculation - split corrections into Loss and Gain
-- Bug: details_v2 was including positive corrections in Loss instead of separating them
-- Fix: Split corrections into corrections_loss (negative) and corrections_gain (positive)
-- Version: v2.11
...
BEGIN
  ...
  -- =============================================
  -- LOSSES (expired, spoiled, other write-offs)
  -- =============================================
  direct_losses AS (
  ...
  -- =============================================
  -- CORRECTIONS - SPLIT INTO LOSS AND GAIN
  -- FIX: Previously all corrections were included in loss
  -- Now: negative corrections = loss, positive corrections = gain
  -- =============================================
  corrections_loss AS (
    SELECT so.operation_date::DATE as loss_date, 'inventory_adjustment' as reason,
      ABS((item->>'quantity')::NUMERIC) as quantity,  -- Use ABS for consistency with report_v3
      ABS(COALESCE((item->>'totalCost')::NUMERIC, 0)) as amount,
      NULL::TEXT as batch_number, so.notes
    FROM storage_operations so, LATERAL jsonb_array_elements(so.items) AS item
    WHERE so.operation_type = 'correction' AND so.operation_date >= p_start_date AND so.operation_date < v_filter_end_date
      ...
      AND (item->>'quantity')::NUMERIC < 0  -- Only negative corrections
  ),
  corrections_gain AS (
    SELECT so.operation_date::DATE as gain_date, 'inventory_adjustment' as reason,
    ...
    WHERE so.operation_type = 'correction' AND so.operation_date >= p_start_date AND so.operation_date < v_filter_end_date
      AND so.status = 'confirmed' AND (item->>'itemId')::UUID = p_product_id
      AND (item->>'quantity')::NUMERIC > 0  -- Only positive corrections
  ),

  -- Loss = direct_losses + corrections_loss (negative corrections only)
  all_losses AS (SELECT * FROM direct_losses UNION ALL SELECT * FROM corrections_loss),
  total_loss AS (SELECT COALESCE(SUM(quantity), 0) as qty, COALESCE(SUM(amount), 0) as amount FROM all_losses),

  -- Gain = positive corrections
  total_gain AS (SELECT COALESCE(SUM(quantity), 0) as qty, COALESCE(SUM(amount), 0) as amount FROM corrections_gain),
  ...
  -- All corrections for analysis (both positive and negative, shown as-is)
  all_corrections_for_analysis AS (
  ...
  corrections_totals AS (
    SELECT COALESCE(SUM(quantity), 0) as qty, COALESCE(SUM(amount), 0) as amount FROM all_corrections_for_analysis
  ),
  ...
  SELECT jsonb_build_object(
    ...
    -- Loss now only includes negative corrections (fixed!)
    'loss', jsonb_build_object('quantity', ROUND((SELECT qty FROM total_loss), 3), 'amount', ROUND((SELECT amount FROM total_loss), 2)),
    -- Gain = positive corrections (new field for consistency)
    'gain', jsonb_build_object('quantity', ROUND((SELECT qty FROM total_gain), 3), 'amount', ROUND((SELECT amount FROM total_gain), 2)),
    ...
    -- Variance formula: Actual - Expected
    ...
    'variance', (SELECT jsonb_build_object('quantity', ROUND(variance_qty, 3), 'amount', ROUND(variance_amount, 2),
        ...
        'possibleReasons', CASE
          ...
          ELSE '[]'::JSONB END)
      FROM (SELECT
        ((SELECT qty FROM raw_stock_totals) + (SELECT qty FROM in_preps_totals))
          - (COALESCE((SELECT quantity FROM opening_snapshot), 0) + (SELECT qty FROM received_totals)
             - (SELECT qty FROM total_sales) - (SELECT qty FROM total_loss) + (SELECT qty FROM total_gain)) as variance_qty,
        ((SELECT amount FROM raw_stock_totals) + (SELECT amount FROM in_preps_totals))
          - (COALESCE((SELECT amount FROM opening_snapshot), 0) + (SELECT amount FROM received_totals)
             - (SELECT qty FROM total_sales) * v_product_avg_cost - (SELECT amount FROM total_loss) + (SELECT amount FROM total_gain)) as variance_amount
      ) v),
    ...
    'actualWriteOffs', jsonb_build_object(
      ...
      'corrections', jsonb_build_object(
        ...
        ) ORDER BY corr_date DESC) FROM all_corrections_for_analysis LIMIT 10), '[]'::JSONB)
      ),
