-- Migration: 103_product_variance_details_v2_rpc
...
--
-- Fixes applied:
--   - Cast po.supplier_id to UUID when joining with counteragents
--   - Use receipt_date instead of received_date for storage_batches
--   - Use production_date instead of produced_at for preparation_batches
...
BEGIN
  ...
  prep_recipes AS (
    SELECT p.id as preparation_id, p.name as preparation_name, p.output_quantity, pi.quantity as product_qty_per_batch
  ...
  products_via_preps AS (
    SELECT pr.preparation_id, pr.preparation_name, SUM(apc.qty * pr.product_qty_per_batch / NULLIF(pr.output_quantity, 0)) as product_used
  ...
  production_batches AS (
    SELECT pr.preparation_id, pr.preparation_name,
      ...
      COALESCE(SUM((item->>'quantity')::NUMERIC * pr.product_qty_per_batch / NULLIF(pr.output_quantity, 0)), 0) as product_used
  ...
  -- Note: preparation_batches uses production_date (not produced_at)
  in_preps AS (
    SELECT pb.preparation_id, p.name as preparation_name, pb.production_date::DATE as batch_date,
      pb.current_quantity * pr.product_qty_per_batch / NULLIF(pr.output_quantity, 0) as product_qty,
      pb.current_quantity * pr.product_qty_per_batch / NULLIF(pr.output_quantity, 0) * v_product_avg_cost as product_cost
...
-- Add comment
COMMENT ON FUNCTION get_product_variance_details_v2 IS 'Enhanced product variance details with full breakdown and drill-down capabilities for V2 dialog';
