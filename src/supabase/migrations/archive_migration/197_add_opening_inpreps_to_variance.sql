-- Migration: 197_add_opening_inpreps_to_variance
-- Description: Add Opening InPreps (products frozen in preparations at period start) to variance report
-- Date: 2026-03-10
--
-- CONTEXT: Opening stock currently only includes raw stock from inventory_snapshots.
-- Products frozen in preparations at the start of the period are NOT counted.
-- This causes the Expected formula to undercount, making variance look more positive.
--
-- Fix: Estimate historical InPreps from preparation_batches:
-- - Active batches: use current_quantity (lower bound)
-- - Depleted batches that were active on opening_date: use initial_quantity (upper bound)
-- Note: This is an approximation. For exact historical data, snapshots should be extended.
--
-- Changes:
-- 1. Create calc_opening_inpreps_bulk(date) helper function
-- 2. Update get_product_variance_report_v4 to include opening InPreps
-- 3. Update get_product_variance_details_v3 to show opening breakdown

-- ============================================================================
-- PART 1: Helper function for historical InPreps
-- ============================================================================

CREATE OR REPLACE FUNCTION calc_opening_inpreps_bulk(p_opening_date DATE)
...
BEGIN
  ...
  prep_batch_tree AS (
    SELECT pb.preparation_id as root_prep_id, pb.preparation_id as current_prep_id,
           (CASE WHEN pb.status = 'active' THEN pb.current_quantity
                 ELSE pb.initial_quantity END)::NUMERIC as batch_qty, 1 as depth
    FROM preparation_batches pb
    WHERE pb.production_date::DATE <= p_opening_date
      AND (pb.status = 'active' OR pb.updated_at::DATE > p_opening_date)
  ...
  SELECT pr.product_id as item_id,
         SUM(pbt.batch_qty * pr.recipe_product_qty / NULLIF(pr.prep_output_quantity, 0))::NUMERIC as qty,
         SUM(pbt.batch_qty * pr.recipe_product_qty / NULLIF(pr.prep_output_quantity, 0)
           * COALESCE(pc.avg_cost, 0))::NUMERIC as amount
  FROM prep_batch_tree pbt
  JOIN prep_recipes pr ON pbt.current_prep_id = pr.preparation_id
  LEFT JOIN product_costs pc ON pc.product_id = pr.product_id
  GROUP BY pr.product_id;
END;
$$;

-- ============================================================================
-- PART 2: Update main variance report to include opening InPreps
-- ============================================================================
...
BEGIN
  ...
  -- Prep recipes (moved up - needed by opening_inpreps and later CTEs)
  prep_recipes AS (
    SELECT p.id as preparation_id,
           (p.output_quantity * COALESCE(p.portion_size, 1))::NUMERIC as prep_output_quantity,
           pi.ingredient_id::TEXT as product_id, pi.quantity::NUMERIC as recipe_product_qty
  ...
  -- NEW: Opening InPreps (products frozen in preparations at period start)
  opening_inpreps AS (
  ...
  -- Current InPreps (for Closing/Actual)
  ...
  product_report AS (
    SELECT
      ...
      COALESCE(oi.qty, 0) as opening_inpreps_qty, COALESCE(oi.amount, 0) as opening_inpreps_amount,
      COALESCE(os.qty, 0) + COALESCE(oi.qty, 0) as opening_qty,
      COALESCE(os.amount, 0) + COALESCE(oi.amount, 0) as opening_amount,
      -- Received
      ...
      (COALESCE(os.qty, 0) + COALESCE(oi.qty, 0) + COALESCE(r.qty, 0) - COALESCE(ts.qty, 0)
        - COALESCE(clw.qty, 0) - COALESCE(cl.qty, 0) + COALESCE(cg.qty, 0)) as expected_qty,
      (COALESCE(os.amount, 0) + COALESCE(oi.amount, 0) + COALESCE(r.amount, 0) - COALESCE(ts.qty, 0) * pi.avg_cost
        - COALESCE(clw.amount, 0) - COALESCE(cl.amount, 0)
        - (COALESCE(clw.qty, 0) - COALESCE(dl.qty, 0)) * pi.avg_cost
        + COALESCE(cg.amount, 0)) as expected_amount,
      -- Actual = Closing + Current InPreps
    ...
    WHERE (COALESCE(os.qty, 0) != 0 OR COALESCE(oi.qty, 0) > 0 OR COALESCE(r.qty, 0) > 0 OR COALESCE(ts.qty, 0) > 0 OR
...
-- ============================================================================
-- PART 3: Update detail function to show opening breakdown
-- ============================================================================
...
BEGIN
  ...
  -- NEW: Opening InPreps with per-preparation detail
  prep_recipes AS (
  ...
  SELECT jsonb_build_object(
    ...
    'period', jsonb_build_object('dateFrom', p_start_date::DATE, 'dateTo', p_end_date::DATE),
    'opening', jsonb_build_object(
      'quantity', COALESCE((SELECT quantity FROM opening_snapshot), 0) + (SELECT qty FROM opening_inpreps_totals),
      'amount', COALESCE((SELECT amount FROM opening_snapshot), 0) + (SELECT amount FROM opening_inpreps_totals),
      ...
      'inPreparations', jsonb_build_object(
        'quantity', ROUND((SELECT qty FROM opening_inpreps_totals), 3),
        'amount', ROUND((SELECT amount FROM opening_inpreps_totals), 2),
        'isEstimate', true,
    ...
    'variance', (
      SELECT jsonb_build_object(
        ...
        SELECT
          -- Expected = Opening(raw+inPreps) + Received - Sales - Loss + Gain
          COALESCE((SELECT quantity FROM opening_snapshot), 0) + (SELECT qty FROM opening_inpreps_totals)
          ...
          COALESCE((SELECT amount FROM opening_snapshot), 0) + (SELECT amount FROM opening_inpreps_totals)
