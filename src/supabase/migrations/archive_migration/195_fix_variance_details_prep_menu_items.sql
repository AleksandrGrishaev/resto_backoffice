-- Migration: 195_fix_variance_details_prep_menu_items
-- Description: Fix detail dialog showing "No sales" for products sold via preparations
-- Date: 2026-03-10
-- Author: Claude
--
-- BUG: get_product_variance_details_v3 shows "No sales in this period" for products
-- consumed through preparations, despite correct totals (1,980g via preps).
--
-- ROOT CAUSE:
-- 1. prep_direct_menu_items only handles Menu → Prep(direct) → Product path
-- 2. Missing CTE for Menu → Recipe → Prep → Product path (most common!)
-- 3. prep_direct_menu_items has same portion_size bug fixed in migration 194
--
-- FIX: Add prep_via_recipe_menu_items CTE + fix portion_size in prep_direct_menu_items
...
BEGIN
  ...
  -- FIX: multiply by portion_size to convert composition_qty (portions) to grams
  ...
  -- 4. NEW: Product via prep-in-recipe in menu items (Menu → Recipe → Prep → Product)
  -- This is the MOST COMMON path that was completely missing!
  ...
  SELECT jsonb_build_object(
    ...
    -- Writeoffs using helper (decomposed from preps)
    ...
    -- Loss using helper (decomposed from preps)
...
COMMENT ON FUNCTION get_product_variance_details_v3 IS 'Product variance details V3.1 - Added Menu->Recipe->Prep->Product path for menu items breakdown. Fixed portion_size bug in prep_direct_menu_items.';
