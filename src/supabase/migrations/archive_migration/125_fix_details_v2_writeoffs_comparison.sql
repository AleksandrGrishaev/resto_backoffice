-- Migration 125: Fix actualWriteOffs comparison in details_v2
-- Problem: differenceFromTheoretical compared (sales + production) with theoretical_sales
-- This is wrong because production_consumption goes INTO preps (InPreps), not lost
--
-- Fix: Compare ONLY sales_consumption with theoretical_sales
-- Production is shown separately for information only
--
-- Version: v2.11
...
BEGIN
  ...
  -- FIX: actual_writeoffs_totals now excludes production_consumption for comparison
  -- production_consumption is shown separately as it goes INTO preps (InPreps), not lost
  actual_writeoffs_totals AS (
    SELECT (SELECT qty FROM sales_consumption_totals) as qty,
           (SELECT amount FROM sales_consumption_totals) as amount
  ),
  ...
  SELECT jsonb_build_object(
    ...
    -- Note: differenceFromTheoretical now compares ONLY salesConsumption with theoretical Sales
    -- productionConsumption is shown separately (it goes into InPreps, not lost)
    'actualWriteOffs', jsonb_build_object(
      ...
      'productionConsumption', jsonb_build_object(
        ...
        'note', 'Product transferred to preparations (InPreps) - not a loss',
        'details', COALESCE((SELECT jsonb_agg(jsonb_build_object(
          'date', wo_date, 'quantity', ROUND(quantity, 3), 'amount', ROUND(amount, 2), 'notes', notes
      ...
      -- FIX: Now compares ONLY sales_consumption with theoretical_sales (not production!)
      'differenceFromTheoretical', (
        ...
        FROM (
          SELECT
            -- FIX: Compare only sales_consumption with total_sales (not production!)
            (SELECT qty FROM sales_consumption_totals) - (SELECT qty FROM total_sales) as diff_qty,
            (SELECT amount FROM sales_consumption_totals) - (SELECT amount FROM total_sales) as diff_amount
        ) diff
      )
    ),
    'generatedAt', NOW()
...
'Product Variance Details V2.11 - Fix: compare only salesConsumption with theoretical Sales, not production';
