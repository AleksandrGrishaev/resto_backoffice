-- Migration: 124_fix_inpreps_portion_size
-- Description: Fix InPreps formula to account for portion_size in preparation decomposition
-- Date: 2026-02-01
-- Author: Claude
--
-- PROBLEM:
-- For portion-type preparations like "Chicken breast thawed 100g":
--   output_quantity = 1 (portion)
--   portion_size = 100 (grams)
--   preparation_batches.current_quantity = 29,900 (grams)
--
-- OLD FORMULA (WRONG):
--   batch_qty * ingredient_qty / output_quantity
--   = 29,900 * 100 / 1 = 2,990,000 grams (3 TONS!)
--
-- NEW FORMULA (CORRECT):
--   batch_qty * ingredient_qty / (output_quantity * COALESCE(portion_size, 1))
--   = 29,900 * 100 / (1 * 100) = 29,900 grams
--
-- CHANGES:
-- 1. prep_recipes CTE: multiply output_quantity by portion_size
-- 2. recursive_prep_decomposition CTE: same fix for nested preps
-- 3. prep_writeoffs_decomposed CTE: same fix
-- 4. prep_loss_decomposed CTE: same fix
-- 5. products_in_preps CTE: same fix
...
BEGIN
  ...
  -- Recipe product sales: Menu -> Recipe -> Product
  ...
  -- Recursive decomposition for nested preparations (for theoretical sales)
  recursive_prep_decomposition AS (
    WITH RECURSIVE prep_tree AS (
      ...
      SELECT pt.root_prep_id, pi.ingredient_id as current_prep_id,
        -- Fix: multiply output_quantity by portion_size for portion-type preps
        (pt.consumed_qty * pi.quantity / NULLIF(p.output_quantity * COALESCE(p.portion_size, 1), 0))::NUMERIC as consumed_qty, pt.depth + 1
      FROM prep_tree pt
  ...
  prep_recipes AS (
    -- CRITICAL: For portion-type preps, total output = output_quantity x portion_size
    -- E.g., "Chicken breast thawed 100g": output_quantity=1 portion, portion_size=100g -> total 100g
  ...
  -- Products from preparations (via preps) - for theoretical sales
  ...
  -- =============================================
  -- WRITE-OFFS (sales_consumption) - DECOMPOSED
  -- Now includes both direct products AND products from prep decomposition
  -- =============================================
  ...
  -- Preparation write-offs (sales_consumption) - need to decompose to products
  prep_writeoffs_raw AS (
    SELECT (item->>'itemId')::UUID as preparation_id,
           SUM((item->>'quantity')::NUMERIC) as qty
    FROM storage_operations so, LATERAL jsonb_array_elements(so.items) AS item
    WHERE so.operation_type = 'write_off'
      AND so.operation_date >= p_start_date
  ...
  -- Decompose preparation write-offs to products (recursive for nested preps)
  prep_writeoffs_decomposed AS (
    WITH RECURSIVE wo_prep_tree AS (
      -- Base: start with prep write-offs
      SELECT pwr.preparation_id as root_prep_id,
  ...
  -- Combined write-offs: direct products + decomposed from preps
  combined_writeoffs AS (
    SELECT item_id, SUM(qty) as qty, SUM(amount) as amount
    FROM (
      SELECT item_id, qty, amount FROM direct_writeoffs
      UNION ALL
      SELECT item_id, qty, 0 as amount FROM prep_writeoffs_decomposed
    ) all_wo
    GROUP BY item_id
  ),

  -- =============================================
  -- LOSS (expired, spoiled, other) - DECOMPOSED
  -- Now includes both direct products AND products from prep decomposition
  -- =============================================
  ...
  -- Preparation losses - need to decompose to products
  prep_loss_raw AS (
    SELECT (item->>'itemId')::UUID as preparation_id,
           SUM((item->>'quantity')::NUMERIC) as qty
    FROM storage_operations so, LATERAL jsonb_array_elements(so.items) AS item
  ...
  -- Decompose preparation losses to products (recursive for nested preps)
  prep_loss_decomposed AS (
    WITH RECURSIVE loss_prep_tree AS (
      -- Base: start with prep losses
      SELECT plr.preparation_id as root_prep_id,
  ...
  -- Combined loss from direct products and decomposed preps
  combined_loss_from_writeoffs AS (
    SELECT item_id, SUM(qty) as qty, SUM(amount) as amount
    FROM (
      SELECT item_id, qty, amount FROM direct_loss
      UNION ALL
      SELECT item_id, qty, 0 as amount FROM prep_loss_decomposed
    ) all_loss
    GROUP BY item_id
  ),
  ...
  -- This is the CRITICAL InPreps calculation - tracks products currently in prep batches
  products_in_preps AS (
    WITH RECURSIVE prep_batch_tree AS (
      ...
      SELECT pbt.root_prep_id, pi.ingredient_id as current_prep_id,
        -- Fix: multiply output_quantity by portion_size for portion-type preps
        (pbt.batch_qty * pi.quantity / NULLIF(p.output_quantity * COALESCE(p.portion_size, 1), 0))::NUMERIC as batch_qty, pbt.depth + 1
  ...
  -- Product report with sales breakdown and decomposed writeoffs/loss
  product_report AS (
    SELECT
      ...
      -- Write-offs (now decomposed from preps!)
      ...
      -- Loss and gain (loss now decomposed from preps!)
...
COMMENT ON FUNCTION get_product_variance_report_v3(TIMESTAMPTZ, TIMESTAMPTZ, TEXT) IS
'Product Variance Report V4.9 - Fix InPreps formula: account for portion_size in prep decomposition';
