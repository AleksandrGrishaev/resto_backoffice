-- Migration: 196_fix_variance_details_writeoffs_loss
-- Description: Fix detail dialog: production write-offs aggregation, loss tracedFromPreps breakdown
-- Date: 2026-03-10
-- Author: Claude
--
-- FIXES:
-- 1. Production write-offs: aggregate by DATE instead of per-row (removes LIMIT 10 cutoff)
-- 2. Add tracedFromPreps to loss section with per-preparation breakdown
-- 3. Remove corrections from actualWriteOffs (already in loss section)
...
BEGIN
  ...
  -- Use helper for theoretical sales (GUARANTEED same as main report)
  sales_data AS (
  ...
  -- Preparations that directly use this product
  prep_recipes AS (
  ...
  -- 1. Direct product in menu items
  ...
  -- 2. Product via recipe in menu items
  ...
  -- 3. Product via DIRECT prep in menu items (Menu -> Prep -> Product)
  prep_direct_menu_items AS (
    SELECT vc.menu_item_id, vc.menu_item_name, vc.variant_name,
      SUM(oid.order_qty) as quantity_sold,
      SUM(oid.order_qty * vc.composition_qty
        * COALESCE(pr.portion_size, 1)
  ...
  -- 4. Product via prep-in-recipe (Menu -> Recipe -> Prep -> Product)
  ...
  all_menu_items AS (
    ...
    UNION ALL
    SELECT menu_item_id, menu_item_name, variant_name, quantity_sold, product_used, sales_type, preparation_name as prep_name
    FROM prep_via_recipe_menu_items
  ),
  ...
  -- Use helpers (GUARANTEED same as report!)
  writeoffs_data AS (
  ...
  -- NEW: Per-preparation loss breakdown (for tracedFromPreps)
  prep_loss_raw_for_breakdown AS (
    SELECT
      (item->>'itemId')::UUID as preparation_id,
      SUM((item->>'quantity')::NUMERIC) as prep_qty
  ...
  -- Decompose prep losses to this product, keeping prep identity
  prep_loss_per_preparation AS (
    SELECT
      pr.preparation_name,
      SUM(plr.prep_qty * pr.product_qty_per_batch
        / NULLIF(pr.output_quantity * COALESCE(pr.portion_size, 1), 0)) as loss_qty
    ...
    HAVING SUM(plr.prep_qty * pr.product_qty_per_batch
      / NULLIF(pr.output_quantity * COALESCE(pr.portion_size, 1), 0)) > 0
    ORDER BY SUM(plr.prep_qty * pr.product_qty_per_batch
      / NULLIF(pr.output_quantity * COALESCE(pr.portion_size, 1), 0)) DESC
  ),
  ...
  -- FIX: Aggregate production write-offs BY DATE (was per-row with LIMIT 10)
  ...
  -- FIX: Aggregate production write-offs BY DATE (was per-row with LIMIT 10, cutting off Mar 1-4)
  ...
  -- Inventory corrections (kept for Loss section)
  ...
  SELECT jsonb_build_object(
    ...
    -- Writeoffs (decomposed from preps)
    ...
    -- Actual write-offs analysis (sales + production, NO corrections - those are in loss)
    'actualWriteOffs', jsonb_build_object(
      ...
      'productionConsumption', jsonb_build_object(
        ...
        -- FIX: aggregated by date, no LIMIT needed
        'details', COALESCE((SELECT jsonb_agg(jsonb_build_object(
          'date', wo_date,
          'quantity', ROUND(qty, 3),
          'amount', ROUND(amount, 2),
          'notes', ops_count || ' operation(s)'
        ) ORDER BY wo_date) FROM production_consumption_writeoffs), '[]'::jsonb)
      ),
    ...
    'loss', jsonb_build_object(
      ...
      -- NEW: Per-preparation loss breakdown
      'tracedFromPreps', jsonb_build_object(
        'quantity', ROUND((SELECT from_preps_loss_qty FROM loss_data), 3),
        ...
        'preparations', COALESCE((SELECT jsonb_agg(jsonb_build_object(
          'preparationName', preparation_name,
          'lossQuantity', ROUND(loss_qty, 3),
          'lossAmount', ROUND(loss_qty * v_product_avg_cost, 2)
        )) FROM prep_loss_per_preparation), '[]'::jsonb)
      )
    ),
    ...
    'variance', (
      SELECT jsonb_build_object(
        ...
        SELECT
          COALESCE((SELECT quantity FROM opening_snapshot), 0) + (SELECT qty FROM received_totals) - (SELECT total_qty FROM sales_data) - (SELECT total_loss_qty FROM loss_data) + (SELECT corrections_gain_qty FROM loss_data) - (SELECT qty FROM raw_stock_totals) - (SELECT qty FROM in_preps_data) as variance_qty,
          COALESCE((SELECT amount FROM opening_snapshot), 0) + (SELECT amount FROM received_totals) - (SELECT total_qty FROM sales_data) * v_product_avg_cost - (SELECT total_loss_qty FROM loss_data) * v_product_avg_cost + (SELECT corrections_gain_qty FROM loss_data) * v_product_avg_cost - (SELECT amount FROM raw_stock_totals) - (SELECT amount FROM in_preps_data) as variance_amount
      ) calc
    ),
    'generatedAt', NOW(),
    'version', 'v3.2'
...
COMMENT ON FUNCTION get_product_variance_details_v3 IS 'Product variance details V3.2 - Production write-offs aggregated by date (no cutoff), tracedFromPreps breakdown, corrections removed from actualWriteOffs.';
