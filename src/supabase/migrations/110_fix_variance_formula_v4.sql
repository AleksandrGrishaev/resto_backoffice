-- Migration: 110_fix_variance_formula_v4
-- Description: Fix variance calculation formula
-- Date: 2026-01-28
--
-- NEW FORMULA:
...
-- - Loss = expired/spoiled + negative corrections (found less than system)
-- - Gain = positive corrections (found more than system)
-- - Closing = Actual (for display, not in formula)
...
BEGIN
  ...
  products_in_preps AS (
    SELECT pr.product_id as item_id,
           SUM(pb.current_quantity * pr.recipe_product_qty / NULLIF(pr.prep_portion_size, 0)) as qty,
           SUM(pb.current_quantity * pr.recipe_product_qty / NULLIF(pr.prep_portion_size, 0) * pi.avg_cost) as amount
...
'Product Variance Report V4 - Expected vs Actual with proper corrections handling';
