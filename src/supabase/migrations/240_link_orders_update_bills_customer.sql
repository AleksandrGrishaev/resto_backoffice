-- Migration: 240_link_orders_update_bills_customer
-- Description: Also update customerName inside bills JSONB when linking anonymous orders
-- Date: 2026-03-20
--
-- PROBLEM: orders.bills is a JSONB array where each bill has customerId/customerName.
-- When link_anonymous_orders or upgrade trigger updates orders.customer_id + customer_name,
-- the bills JSONB still has customerName=null. POS checkout reads from bills.

-- ============================================================
-- 1. Update link_anonymous_orders
-- ============================================================
...
BEGIN
  ...
  -- Link orders: update customer_id, customer_name, AND bills JSONB
  WITH target_orders AS (
    UPDATE orders
    SET customer_id = v_customer_id,
        customer_name = COALESCE(v_customer_name, customer_name),
        -- Update customerName in each bill of the JSONB array
        bills = (
  ...
  SELECT count(*) INTO v_linked_count FROM target_orders;
...
-- ============================================================
-- 2. Update handle_auth_user_upgrade
-- ============================================================
...
BEGIN
  ...
  -- Link anonymous orders: update customer_id, customer_name, AND bills JSONB
...
-- ============================================================
-- 3. Fix existing orders: backfill customerName in bills JSONB
-- ============================================================
UPDATE orders
SET bills = (
  SELECT jsonb_agg(
    bill || jsonb_build_object('customerName', orders.customer_name)
  )
  FROM jsonb_array_elements(orders.bills) AS bill
)
WHERE customer_name IS NOT NULL
  AND bills IS NOT NULL
  AND jsonb_array_length(bills) > 0
  AND bills->0->>'customerName' IS NULL;
